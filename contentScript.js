...
function insertIntoWebpage(pwd){
    console.log("Generated Password: " + pwd);
    console.log("inserting into webpage...");
    pwdfield.value = pwd;
}
...
function loginFunction(){
  ...
  //Check Checkbox
  if(document.getElementById('checkboxForPasswordGenerator38x44').checked){
    //Get data from the page...
        
    let pwd = document.getElementById('pwdfield').value;
    let url = fixUrl(window.location.href);
    generatePwd(url, pwd);
    //var pwd = generatePwd('', '');
  }
  else{
    console.log("Checkbox on False!");
  }
}
...
for(i = 0; i < inputList.length; i++){
  if(inputList[i].getAttribute("type") == "password"){
      pwdfield = inputList[i];
  }
}

pwdfield.onblur = function() {loginFunction()};


var div = document.createElement('div');
div.innerHTML = 
'<label class="switch"><input type="checkbox" id="checkboxForPasswordGenerator38x44" checked><span class="slider round"><p style="margin-top: 7px; margin-left: 10px;">P</p></span></label>';
document.getElementById("submitBtn").parentNode.appendChild( div );

//var popupaccess = fixUrl("Just");
